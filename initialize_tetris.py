...
class Main:
    ...
    def draw_grid(self, surface, grid):
        ...
        for i in range(len(grid)): 
            pygame.draw.line(surface, (128,128,128), (sx,sy+i*self.settings.block_size),(sx+self.settings.play_width,sy + i*self.settings.block_size))
            for j in range(len(grid[i])): 
                pygame.draw.line(surface, (128,128,128), (sx + j*self.settings.block_size,sy),\
